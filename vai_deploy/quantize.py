...
parser.add_argument(
    ...
    ' (default: pixel1_lat@143ms_top1@80.1_finetune@75)')
parser.add_argument('--deploy', action='store_true', help='Deploy the quantized network')
...
# Set the visible cuda devices before importing nndct
import pytorch_nndct as nndct

def quantize(net, val_data):
    net.eval()
    
    data = torch.randn([args.batch_size, 3, image_size, image_size]).cuda()
    quantizer = nndct.torch_quantizer('calib', net, data)
    ...
    num_iter = 10
    with torch.no_grad():
    	with tqdm(total=num_iter, desc='Calibrating') as t:
    		for i, (images, labels) in zip(range(num_iter), val_data):
...
def test(net, val_data):
    ...
    data = torch.randn([args.batch_size, 3, image_size, image_size]).cuda()
    quantizer = nndct.torch_quantizer('test', net, data)
    ...
    with torch.no_grad():
        with tqdm(total=len(val_data), desc='Validate') as t:
    ...
    print('Test OFA quantized specialized net <%s> with image size %d:' % (args.net, image_size))
    print('Results: top1=%.1f,\t top5=%.1f' % (top1.avg, top5.avg))
